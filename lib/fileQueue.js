...
var queue = function(spec, my) {
  ...
  init = function(execute) {
    ...
    var fire = function(file, i) {
      setTimeout(function() {
        console.log('firing requeue: %d %s', i, file);
  ...
  retry = function(filename) {
    ...
    fs.readFile(my.filePath + '/' + filename, 'utf-8', function(err, data) {
      ...
      my.execute(request.op, request.data, postExec, true);
    });
  };
